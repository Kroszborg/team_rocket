...
---
This report was generated by Virtual Campaign Lab
...
export function shareToTwitter(data: ExportData): void {
  const { campaign, results } = data;
  const text = `Just tested my marketing campaign "${campaign.name}" and got ${results.metrics.estimatedROI}% ROI! 🚀 ${results.metrics.estimatedReach.toLocaleString()} reach with Virtual Campaign Lab. Practice smart, spend smarter! 💡`;
...
export function shareToLinkedIn(data: ExportData): void {
  ...
  const summary = `Tested my marketing campaign and achieved ${results.metrics.estimatedROI}% ROI with ${results.metrics.estimatedReach.toLocaleString()} estimated reach using Virtual Campaign Lab.`;
